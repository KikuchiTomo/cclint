-- クラスメソッド内でのdeleteキーワードの使用を禁止する（RAIIを使用すること）
rule_description = "クラスメソッド内でdeleteを使用しない"
            ...
            for _, method_name in ipairs(methods) do
                ...
                if method_info then
                    ...
                    for line_num = start_line, end_line do
                        ...
                        if line and line:match("[^%w_]delete%s+") then
                            cclint.report_warning(
                                line_num, 1,
                                string.format("クラス '%s' のメソッド '%s' でdeleteを使用しないでください。RAIIとスマートポインタを使用してください。",
