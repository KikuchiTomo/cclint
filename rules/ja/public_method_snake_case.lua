-- publicメソッドはsnake_caseを使用すること (例: get_value, set_data)
rule_description = "publicメソッドはsnake_caseを使用すること"
            ...
            if method_info.access == 0 then
                ...
                if not method_name:match("^[a-z][a-z0-9_]*$") then
                    cclint.report_warning(
                        method_info.line, 1,
                        string.format("publicメソッド '%s' はsnake_caseにしてください (例: get_value)", method_name)
                    )
                end
            end
