-- protectedメソッドはsnake_caseを使用すること (例: compute_value)
rule_description = "protectedメソッドはsnake_caseを使用すること"
            ...
            if method_info.access == 1 then
                ...
                if not method_name:match("^[a-z][a-z0-9_]*$") then
                    cclint.report_warning(
                        method_info.line, 1,
                        string.format("protectedメソッド '%s' はsnake_caseにしてください (例: compute_value)", method_name)
                    )
                end
            end
