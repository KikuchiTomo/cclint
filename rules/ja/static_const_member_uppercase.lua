-- static constメンバ変数はUPPER_CASEを使用すること (例: MAX_SIZE, DEFAULT_VALUE)
rule_description = "static constメンバはUPPER_CASEを使用すること"
                    ...
                    if field_info.is_static and field_info.is_const then
                        ...
                        if not field_name:match("^[A-Z][A-Z0-9_]*$") then
                            cclint.report_warning(
                                field_info.line, 1,
                                string.format("static constメンバ '%s' はUPPER_CASEにしてください (例: %s)",
