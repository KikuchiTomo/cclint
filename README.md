...
libclang を同梱したバイナリ tarball をリリースしています．

```bash
curl -fsSL https://raw.githubusercontent.com/tomokikuchi/cclint/main/scripts/install.sh | bash
```

`$HOME/.local/bin/cclint` に入ります．インストール先を変えたいときは:

```bash
CCLINT_PREFIX=/usr/local curl -fsSL .../install.sh | sudo bash
```

対応プラットフォーム: Linux x86_64 / Linux aarch64 / macOS x86_64 / macOS aarch64
