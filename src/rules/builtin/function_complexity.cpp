...
void FunctionComplexityRule::check_file(
    ...
    while (std::regex_search(search_start, content.cend(), match, func_pattern)) {
        ...
        if (complexity > max_complexity_) {
            ...
            int func_line = 1 + std::count(content.begin(),
                                          content.begin() + match_pos, '\n');

            std::ostringstream msg;
            msg << "Function '" << func_name
                << "' has cyclomatic complexity of " << complexity
...
int FunctionComplexityRule::calculate_complexity(
...
int FunctionComplexityRule::count_control_statements(
    ...
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), if_pattern),
        std::sregex_iterator());

    // else if
    std::regex elif_pattern(R"(\belse\s+if\s*\()");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), elif_pattern),
        std::sregex_iterator());

    // for
    std::regex for_pattern(R"(\bfor\s*\()");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), for_pattern),
        std::sregex_iterator());

    // while
    std::regex while_pattern(R"(\bwhile\s*\()");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), while_pattern),
        std::sregex_iterator());

    // do-while
    std::regex do_pattern(R"(\bdo\s*\{)");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), do_pattern),
        std::sregex_iterator());

    // switch case
    std::regex case_pattern(R"(\bcase\s+)");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), case_pattern),
        std::sregex_iterator());

    // catch
    std::regex catch_pattern(R"(\bcatch\s*\()");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), catch_pattern),
        std::sregex_iterator());

    // ternary operator
    std::regex ternary_pattern(R"(\?[^:]+:)");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), ternary_pattern),
        std::sregex_iterator());

    // logical && and ||
    std::regex and_pattern(R"(&&)");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), and_pattern),
        std::sregex_iterator());

    std::regex or_pattern(R"(\|\|)");
    count += std::distance(
        std::sregex_iterator(code.begin(), code.end(), or_pattern),
        std::sregex_iterator());
