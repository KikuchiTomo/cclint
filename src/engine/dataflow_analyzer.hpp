...
struct DataFlowAnalysisResult {
    ...
    std::map<std::string, std::vector<int>> null_pointer_risks;  // null参照リスク（変数名 -> 行番号）
};
...
public:
    ...
    /// @param ast 解析対象のAST
    /// @return 未初期化変数のセット
    std::set<std::string> detect_uninitialized_variables(
        std::shared_ptr<parser::TranslationUnitNode> ast);

    /// null参照リスクを検出
    /// @param ast 解析対象のAST
    /// @return null参照リスクのマップ（変数名 -> 行番号のリスト）
    std::map<std::string, std::vector<int>> detect_null_pointer_risks(
        std::shared_ptr<parser::TranslationUnitNode> ast);
    ...
    /// @return 診断メッセージのリスト
    std::vector<diagnostic::Diagnostic> generate_diagnostics(
