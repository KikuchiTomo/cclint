...
function check_file()
    ...
    for line_num, line in ipairs(file_lines) do
        ...
        -- Check for std::cout usage
        if in_class and line:match("std::cout") then
            cclint.report_warning(
                line_num, 1,
                string.format("Do not use std::cout in class '%s'. Use a logger instead.", class_name or "unknown")
            )
        end
    end
end
