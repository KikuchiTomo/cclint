...
function check_file()
    ...
    for line_num, line in ipairs(file_lines) do
        ...
        if in_class and line:match("[^%w_]new%s+[%w_]") then
            cclint.report_warning(
                line_num, 1,
                string.format("Do not use 'new' in class '%s'. Use std::make_unique or std::make_shared instead.", class_name or "unknown")
            )
        end
    end
end
